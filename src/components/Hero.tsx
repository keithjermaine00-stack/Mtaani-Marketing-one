...
type Phase = 'intro' | 'campaign' | 'goal' | 'settled';
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    timers.push(setTimeout(() => setPhase('campaign'), 400));
  ...
  const getCardStyle = (card: 'campaign' | 'goal') => {
    const visible = phase === 'settled' ||
      (card === 'campaign' && (phase === 'campaign' || phase === 'goal')) ||
      (card === 'goal' && phase === 'goal');
    const onCenter = visible && phase !== 'settled';
    const extraY = card === 'goal' && !visible ? 40 : 0;
    ...
    return {
      ...
      zIndex: card === 'goal' ? 20 : 10,
    };
  };

  const bgVisible = phase !== 'intro';

  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden bg-spotify-dark-base">
        ...
        <div className="grid lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_460px] gap-10 xl:gap-16 items-center">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="font-black tracking-tighter leading-[0.92] mb-7"
              ...
              <span className="block gradient-text overflow-hidden"
            ...
            <div ref={cardsRef} className="relative mb-8 h-[260px] sm:h-[280px]" style={{ zIndex: 30 }}>
              {/* Campaign Report card */}
              <div className="absolute top-0 left-0 glass-card rounded-2xl p-5 w-[270px] sm:w-[300px]" style={getCardStyle('campaign')}>
                <p className="text-white/50 text-xs font-semibold uppercase tracking-wider mb-3">Campaign Report</p>
                <div className="flex items-baseline gap-3 mb-1">
                  <span className="text-white font-black text-3xl">{impressions.toLocaleString()}</span>
                  <span className="flex items-center gap-1 text-spotify-green text-xs font-bold bg-spotify-green/15 px-2 py-0.5 rounded-full">
                    <ArrowUpRight size={11} />23%
                  </span>
                ...
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[10px] text-white font-semibold bg-white/10 px-2 py-0.5 rounded-full">Target: 13K</span>
                          ...
                          {d.heights.map((h, bi) => (
                            <div key={bi}
                              className={`w-[6px] rounded-sm transition-all duration-700 ${
                                hi ? (bi === 2 ? 'bg-spotify-green' : 'bg-spotify-green/50') : (bi === 2 ? 'bg-white/30' : 'bg-white/12')
                              }`}
                              style={{ height: active ? `${h * 0.56}px` : '4px', transitionDelay: `${gi * 0.08 + bi * 0.03}s` }}
                            />
                          ))}
                        </div>
              ...
              <div className="absolute bottom-0 left-[230px] sm:left-[280px] rounded-2xl p-5 w-[200px] sm:w-[220px]"
                style={{
                  ...getCardStyle('goal'),
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-black/70 text-xs font-bold uppercase tracking-wider">Goal Achievement</p>
                </div>
                <div className="flex items-baseline gap-3 mb-1">
                  <span className="text-black font-black text-2xl">{goalVal.toLocaleString()}</span>
                  <span className="flex items-center gap-1 bg-black/15 text-black/80 text-xs font-bold px-2 py-0.5 rounded-full">
                    <ArrowUpRight size={10} />54%
                  </span>
                </div>
                <p className="text-black/50 text-[11px]">October 2025</p>
                <div className="mt-3 h-1.5 rounded-full bg-black/15 overflow-hidden">
                  <div className="h-full rounded-full bg-black/40 transition-all duration-1000 ease-out"
                    style={{ width: active ? '54%' : '0%', transitionDelay: '0.3s' }} />
                </div>
              </div>
            </div>

            <p className="text-spotify-subdued text-base sm:text-lg leading-relaxed max-w-md mb-8"
            ...
            <div className="flex flex-wrap gap-3"
              ...
              <a href="#contact" className="btn-outline text-sm px-7 py-3.5">Let's Talk</a>
            </div>
          </div>

          {/* ── RIGHT – abstract visual panel ── */}
          <div className="hidden lg:block"
            style={{ animation: 'fadeUp 0.8s cubic-bezier(.16,1,.3,1) forwards', animationDelay: '2.2s', opacity: 0 }}>
            <div className="relative">
              ...
              <div className="relative rounded-3xl overflow-hidden" style={{ height: '580px' }}>
                ...
                <div className="absolute top-6 right-6 glass-dark rounded-2xl px-4 py-3"
                  style={{ animation: 'fadeUp 0.6s ease forwards', animationDelay: '2.6s', opacity: 0 }}>
                ...
                <div className="absolute bottom-6 left-6 right-6 glass-dark rounded-2xl px-4 py-3 flex items-center justify-between"
                  style={{ animation: 'fadeUp 0.6s ease forwards', animationDelay: '2.8s', opacity: 0 }}>
                  <div>
  ...
  return (
    <>
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 580" preserveAspectRatio="none">
        ...
        <path d="M-50,150 Q100,100 200,180 T450,160 L450,0 L-50,0 Z" fill="url(#w1)" />
        <path d="M-50,300 Q120,250 220,320 T450,290 L450,200 T-50,220 Z" fill="url(#w2)" />
        <path d="M-50,500 Q100,440 200,500 T450,470 L450,580 L-50,580 Z" fill="url(#w3)" />
